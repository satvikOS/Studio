...
SCRIPTS = {
 ...
 '04': "Don't take our word for it. Watch. On the left, Forge builds a real jet engine, the whole process. On the right, "
       "Studio walks you through a living forest, in real time. One sentence each. No cloud, no waiting. This is real, "
       "and it runs today.",
 '05': "And it is not only engineering. This is Studio. Every one of these rooms began as a single line. A cozy bedroom. "
       "A bright kitchen. A little cafe. Archie laid out the space, placed the furniture, and lit the scene. What used to "
       "take an artist days now takes a moment.",
 '06': "Why us, and why now. Four things. It is a real kernel, so you get parts you can build, not just a pretty picture. "
 ...
 '07': "And this is not a thin wrapper on someone else's AI. There is real engineering underneath. A native kernel for Forge. "
 ...
 '08': "How we make money is simple, and you have seen it work before. The same as GitHub and Vercel. Free for anyone creating "
 ...
 '09': "Our path is bottom up. We land with students, makers and indie engineers, the people who love to try things for free. "
 ...
 '10': "So here is what we believe. If you can dream it, and you dare to create it, you can make worlds. That used to take "
...
assert len(pngs) == 10, f'expected 10 slides, got {len(pngs)}: {[os.path.basename(p) for p in pngs]}'

# demo slide: two video frames (must match .f-forge / .f-studio rects in 04-demo.html)
DEMO = [
    dict(left=56,  top=244, w=572, h=280, video=os.path.join(BD, 'assets', 'demo-forge.mp4')),
    dict(left=652, top=310, w=572, h=214, video=os.path.join(BD, 'assets', 'demo-studio.mp4')),
]
...
for p in pngs:
    ...
    if os.path.basename(p).startswith('04-demo'):
        im = Image.open(p); sx = im.width / 1280.0
        for i, d in enumerate(DEMO):
            if not os.path.exists(d['video']):
                continue
            poster = os.path.join(BD, 'render', f'_demo_poster{i}.png')
            box = (int(d['left']*sx), int(d['top']*sx), int((d['left']+d['w'])*sx), int((d['top']+d['h'])*sx))
            im.crop(box).save(poster)
            s.shapes.add_movie(d['video'],
                Emu(d['left']*PXEMU), Emu(d['top']*PXEMU), Emu(d['w']*PXEMU), Emu(d['h']*PXEMU),

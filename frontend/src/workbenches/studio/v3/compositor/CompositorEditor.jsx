// ArchDisc Studio V3 — compositor graph editor.
//
// Mirrors ShaderEditor's SVG-based node-graph UI but plugs into the
// compositor surface (window.__studioCompositor*). The header bar
// exposes one button per node kind; sockets drag-connect; the props
// panel edits the selected node's params live; an Evaluate button
// runs the graph and paints the result into the overlay canvas.
//
// Rendered into a body-attached host (no React-tree mutations to
// StudioShellV3.jsx).
...
export default function CompositorEditor({ getGraph, evaluate, onCloseRequest }) {
  ...
  // Auto-evaluate preview when the graph changes (debounced).
  useEffect(() => {
      ...
      for (const n of graph.nodes.values()) {
        ...
        for (let i = 0; i < def.inputs.length; i++) {
          ...
          if (Math.hypot(p.x - x, p.y - y) < 9 && n.id !== d.srcId) {
            if (window.__studioCompositorNodeConnect) {
              window.__studioCompositorNodeConnect(d.srcId, d.srcOut, n.id, def.inputs[i].name);
            }
            break;
          }
        }
  ...
  const addKind = (kind) => {
    if (window.__studioCompositorNodeAdd) {
      const r = window.__studioCompositorNodeAdd(kind, {});
  ...
  const removeSelected = () => {
    ...
    if (window.__studioCompositorNodeRemove) window.__studioCompositorNodeRemove(selectedId);
    setSelectedId(null);
    tick();
  };
  ...
  return (
    <div
      data-studio-v3-compositor-editor
    ...
    >
      ...
      <div style={{
        ...
        {listKinds().map((k) => (
          <button
            key={k}
            data-studio-v3-compositor-add={k}
            onClick={() => addKind(k)}
            style={{
        ...
        <button
          data-studio-v3-compositor-evaluate
          onClick={runEvaluate}
          style={{
        ...
        >Evaluate</button>
        <button
          data-studio-v3-compositor-close
      ...
      <div style={{ display: 'flex', flex: 1, minHeight: 0 }}>
        ...
        >
          ...
          {Array.from(graph.nodes.values()).map((n) => {
            ...
            return (
              <g
                key={n.id}
                data-studio-v3-compositor-node={n.id}
                data-studio-v3-compositor-node-kind={n.kind}
              ...
              >
                ...
                {def.inputs.map((slot, i) => {
                  const p = socketXY(n, def, 'input', i);
                  return (
                    <g key={`in-${slot.name}`} data-studio-v3-compositor-sock-in={slot.name}>
                ...
                {def.outputs.map((slot, i) => {
                  ...
                  return (
                    <g
                      key={`out-${slot.name}`}
                      data-studio-v3-compositor-sock-out={slot.name}
        ...
        {/* Props panel */}
        <div
          data-studio-v3-compositor-props
          style={{
        ...
        >
            ...
            <div>
              ...
              {Object.entries(selected.params).map(([key, value]) => (
                <div key={key} style={{ marginBottom: 8, fontSize: 11 }}>
                  ...
                  {typeof value === 'number' ? (
                    <input
                      data-studio-v3-compositor-param={key}
                      type="number"
                      step="0.05"
                      value={value}
              ...
              <button
                data-studio-v3-compositor-remove
          ...
          {previewUrl ? (
            <div style={{ marginTop: 18 }}>
              <div style={{ fontSize: 11, opacity: 0.6, marginBottom: 4 }}>Composited frame</div>
              <img
                data-studio-v3-compositor-preview
                src={previewUrl} alt="composite"

// ArchDisc Studio V3 — shader graph state + topological evaluator.
//
// The graph stores nodes by uuid and a flat list of wires:
//   { srcId, srcOut, dstId, dstIn }
//
// evaluate(ctx) walks backward from each `output` kind node, resolving
// inputs by recursive eval. Visited nodes are cached per evaluate() call
// so a node feeding two consumers is only evaluated once per pixel.
//
// Multi-output nodes (Geometry, Noise) return an object map keyed by
// output name; the consumer extracts the slot it wired to.
...
export function connect(graph, srcId, srcOut, dstId, dstIn) {
  ...
  // Replace any existing wire targeting (dstId, dstIn) — slot is single-input.
...
// Cycle test: would adding (src → dst) create a path back to src from dst?
...
// Evaluate a single node for the current ctx; returns its output value(s).
// `cache` is keyed by node id (Map<id, value>) so each node fires once per
// pixel even with multiple downstream consumers.
function evalNode(graph, node, ctx, cache, stack) {
  ...
  // Resolve every input by walking upstream.
  const ins = new Map();
  for (const inSlot of def.inputs) {
    ...
    if (wire) {
      ...
      const srcDef = NODE_KINDS[src.kind];
      if (srcDef.multiOutput && upstream && typeof upstream === 'object' && !Array.isArray(upstream)) {
...
// Find the (first) Output node, or null if none.
export function findOutput(graph) {
  for (const n of graph.nodes.values()) if (n.kind === 'output') return n;
  return null;
}

// Evaluate for a single per-pixel ctx, returning the Output node's color.
// If no Output exists, return [0,0,0].
export function evaluatePixel(graph, ctx) {
  const out = findOutput(graph);
  if (!out) return [0, 0, 0];
  const cache = new Map();
  const stack = new Set();
  const v = evalNode(graph, out, ctx, cache, stack);
  // Output.eval returns a color array already.

...
// Slice 314 — three-mesh-bvh prototype patch makes every BufferGeometry
// raycast use a BVH (lazy-built on first call). Fail-soft via try/catch
// so a missing dep falls back to vanilla O(N) raycaster.
let __BVH_OK = false;
try {
  ...
  __BVH_OK = true;
} catch (_) { /* fallback — vanilla raycast still works */ }
...
import { bakeAOToTexture } from './bake/aoTexture.js';
import { bakeNormalToTexture } from './bake/normalTexture.js';
import { bakePositionToTexture } from './bake/positionTexture.js';
import { generateProceduralTexture } from './bake/proceduralTexture.js';
import { projectPaintFromCamera } from './bake/projectPaint.js';
...
function WorkbenchStudioV2({ headless = false } = {}) {
  ...
  useEffect(() => {
    ...
    // instancing at scale.
    // Slice 287 — Maya MASH "Distribute" / C4D Cloner / Houdini copy-to-points.
    ...
    window.__studioSetCameraAxis = (axis) => {
      ...
      const t = (ctrl && ctrl.target) ? ctrl.target : new THREE.Vector3(0, 0, 0);
      const R = 0.25; // distance from target
      const POS = {
        top:    [t.x,         t.y + R,  t.z],
        bottom: [t.x,         t.y - R,  t.z],
        front:  [t.x,         t.y,      t.z + R],
        back:   [t.x,         t.y,      t.z - R],
        side:   [t.x + R,     t.y,      t.z],
        persp:  [t.x + R * 0.6, t.y + R * 0.4, t.z + R * 0.6],
      };
      const p = POS[axis];
      if (!p) return { ok: false, error: 'unknown axis: ' + axis };
      cam.position.set(p[0], p[1], p[2]);
      cam.lookAt(t);
      if (ctrl && ctrl.update) ctrl.update();
      window.__studioCameraAxis = axis;
      return { ok: true, axis, position: [p[0], p[1], p[2]] };
    };
    // Slice 310 — Cinema 4D MoText / 3ds Max Text — imperative API for
    // spawning extruded 3D text without going through the React input UI.
    // Reuses fontRef (loaded once) + TextGeometry; returns the mesh uuid.
    // Slice 312 — Blender Mirror Modifier. Clones the selected mesh and flips
    // its scale on the chosen axis (x/y/z), adds it to the scene as a sibling.
    // Result: instant left-right (or up-down) symmetry.
    // Slice 330 — Blender Ctrl+A Apply Transforms. Bakes the selected mesh's
    // {position,rotation,scale} into the geometry's vertex positions then
    // resets the mesh transform to identity. Critical for export workflows:
    // GLTF/USD consumers often choke on non-trivial parent transforms, so
    // baking guarantees the mesh ships with vertex positions in world space.
    // Slice 332 — Blender "Set Origin → Geometry to Origin" / Maya "Center
    // Pivot". Moves the mesh's origin to the centroid of its vertex cloud
    // (or to a chosen vertex / bbox center) without changing the visual
    // position. Crucial for rotate/scale-around-self workflows where the
    // current origin sits far from the geometry.
    window.__studioRecenterPivot = (opts = {}) => {
      const m = selectedMeshRef.current;
      if (!m || !m.geometry || !m.geometry.attributes.position) return { ok: false, error: 'no mesh' };
      const { mode = 'centroid' } = opts;
      ...
      // Shift every vertex by -c so origin lands at the centroid.
      for (let i = 0; i < pos.count; i++) {
        pos.setXYZ(i, pos.getX(i) - c.x, pos.getY(i) - c.y, pos.getZ(i) - c.z);
      }
      pos.needsUpdate = true;
      // Compensate the mesh position so the visual location stays put.
      // Account for the mesh's rotation + scale when offsetting world position.
      const worldOffset = c.clone().multiply(m.scale).applyEuler(m.rotation);
      m.position.add(worldOffset);
      m.geometry.computeBoundingSphere();
      m.geometry.computeBoundingBox();
      m.updateMatrixWorld(true);
      m.userData.archdiscStudioRecenteredPivot = (m.userData.archdiscStudioRecenteredPivot || 0) + 1;
      ...
      return { ok: true, mode, offset: [c.x, c.y, c.z] };
    };
    window.__studioApplyTransforms = (opts = {}) => {
      const m = selectedMeshRef.current;
      if (!m || !m.geometry) return { ok: false, error: 'no mesh' };
      const { position = true, rotation = true, scale = true } = opts;
      const before = {
      ...
      const savedP = m.position.clone(), savedR = m.rotation.clone(), savedS = m.scale.clone();
      if (!position) m.position.set(0, 0, 0);
      if (!rotation) m.rotation.set(0, 0, 0);
      if (!scale)    m.scale.set(1, 1, 1);
      m.updateMatrix();
      mtx.copy(m.matrix);
      m.geometry.applyMatrix4(mtx);
      m.geometry.computeVertexNormals();
      m.geometry.computeBoundingSphere();
      m.geometry.computeBoundingBox();
      if (position) m.position.set(0, 0, 0); else m.position.copy(savedP);
      if (rotation) m.rotation.set(0, 0, 0); else m.rotation.copy(savedR);
      if (scale)    m.scale.set(1, 1, 1);    else m.scale.copy(savedS);
      m.updateMatrixWorld(true);
      m.userData.archdiscStudioAppliedTransforms = (m.userData.archdiscStudioAppliedTransforms || 0) + 1;
      recomputeMeshStats(window.__archdiscScene);
      setSelectedTransform({
        position: { x: m.position.x, y: m.position.y, z: m.position.z },
        rotation: { x: m.rotation.x, y: m.rotation.y, z: m.rotation.z },
      ...
      return { ok: true, applied: { position, rotation, scale }, before };
    };
    window.__studioMirrorAcrossAxis = (axis) => {
      const m = selectedMeshRef.current;
      const scene = window.__archdiscScene;
      if (!scene || !m || !m.geometry || !m.material) return { ok: false, error: 'no mesh' };
      const a = (axis || 'x').toLowerCase();
      if (a !== 'x' && a !== 'y' && a !== 'z') return { ok: false, error: 'axis must be x/y/z' };
      const clone = m.clone();
      clone.geometry = m.geometry.clone();
      clone.material = m.material.clone();
      // Flipping scale on one axis inverts winding — also flip a normal so
      // backface culling stays correct under MeshStandardMaterial.
      clone.scale.set(
        m.scale.x * (a === 'x' ? -1 : 1),
        m.scale.y * (a === 'y' ? -1 : 1),
        m.scale.z * (a === 'z' ? -1 : 1),
      ...
      clone.userData.archdiscStudioPrimitiveKind = (m.userData && m.userData.archdiscStudioPrimitiveKind) || 'mirror';
      clone.userData.archdiscStudioMirrorOf = m.uuid;
      clone.name = `${m.name || 'studio-primitive'}-mirror-${a}`;
      scene.add(clone);
      if (primitiveStackRef.current) primitiveStackRef.current.push(clone);
      setPrimitiveCount((c) => c + 1);
      return { ok: true, uuid: clone.uuid, axis: a, sourceUuid: m.uuid };
    };
    window.__studioMoText = (opts = {}) => {
      const scene = window.__archdiscScene;
      if (!scene) return { ok: false, error: 'no scene' };
      if (!fontRef.current) return { ok: false, error: 'font not loaded yet' };
      ...
      mesh.userData.archdiscMoText = { text: String(text), size };
      mesh.name = `studio-primitive-motext`;
      scene.add(mesh);
      if (primitiveStackRef.current) primitiveStackRef.current.push(mesh);
      setPrimitiveCount((c) => c + 1);
      return { ok: true, uuid: mesh.uuid, text: String(text), size };
    };
    // Slice 307 — sun-angle control. azimuth (0..2π) and elevation (0..π/2)
    // place the directional key light on a hemisphere of radius 24 around
    // the origin; elevation low = warm rim, high = noon overhead.
    window.__studioSetSunAngle = (opts) => {
      const vp = window.__archdiscViewport;
      if (!vp || !vp.keyLight) return { ok: false, error: 'no key light' };
      const { azimuth = 0, elevation = Math.PI / 4 } = (opts || {});
      const R = 24;
      const ce = Math.cos(elevation), se = Math.sin(elevation);
      const x = R * ce * Math.cos(azimuth);
      const y = R * se;
      const z = R * ce * Math.sin(azimuth);
      vp.keyLight.position.set(x, y, z);
      vp.keyLight.target.position.set(0, 0, 0);
      vp.keyLight.target.updateMatrixWorld();
      window.__studioSunAngle = { azimuth, elevation, position: [x, y, z] };
      return { ok: true, azimuth, elevation, position: [x, y, z] };
    };
    window.__studioMashDistribute = (opts) => {
      ...
      let src = null;
      scene.traverse((o) => { if (o.isMesh && o.uuid === sourceUuid) src = o; });
      if (!src || !src.geometry || !src.material) return null;
      let a = (seed >>> 0) || 1;
      const rng = () => {
        a = (a + 0x6D2B79F5) >>> 0;
        let t = a;
      ...
      setPrimitiveCount((c) => c + 1);
      return { uuid: inst.uuid, count: N, mode };
    };
    ...
    // Slice 292 — Unreal Niagara / Cascade-style continuous particle emitter.
    // Fixed-size ring buffer; rAF-driven tick spawns rate*dt new particles per
    // frame, ages all alive ones, integrates gravity+velocity. Returns a
    // handle with stop/pause/state so the caller (and tests) can drive it.
    window.__studioParticleEmitter = (opts) => {
      const scene = window.__archdiscScene;
      if (!scene) return null;
    ...
    window.__studioMoSpline = (opts = {}) => {
      ...
      scene.add(line);
      if (primitiveStackRef.current) primitiveStackRef.current.push(line);
      setPrimitiveCount((c) => c + 1);
    ...
    window.__studioFilletEdges = (opts) => filletEdges(opts || {});
    // Slice 301 — Plasticity/MoI per-edge listing + highlight pick. MVP
    // by integer index — no raycaster yet. The yellow LineSegments is
    // attached to the mesh so it follows transforms.
    // Slice 350 — Blender / Mari UV seam marking. The user marks edges
    // as seams; a subsequent unwrap walks those seams to cut the mesh
    // into a flat 2D layout. Studio's mark-seam stores a Set of edge
    // keys ("minIdx,maxIdx") on the mesh userData. __studioListSeams
    // returns the keys; __studioClearSeams clears.
    // Slice 351 — Maya/Blender "Look At" constraint. Selected mesh
    // orients its -Z axis toward the target (mesh uuid OR a world point).
    // Uses Three.js Object3D.lookAt — preserves position/scale.
    // Slice 352 — ArchViz "Drop to Ground". Translates the selected mesh
    // straight down (or up) so its bounding-box bottom lands on y=0
    // (or another floor). Common building / asset placement workflow.
    // Slice 353 — 3ds Max Array / Plasticity Replicate. Clones the
    // selected mesh N times along an axis with given spacing. Each
    // clone is a real new mesh (not an instance) so it can be
    // individually edited; userData links back to the source uuid.
    // Slice 354 — Houdini "Scatter" / Cinema 4D "Random Effector" /
    // foliage workflow. Drops N real clones of the selected mesh at
    // random positions within a sphere of radius `radius` around the
    // source mesh. Deterministic via mulberry32(seed).
    // Slice 355 — Maya "Center Object" / Blender "Origin to 3D Cursor +
    // World Origin". Translates the mesh so its bounding-box centre
    // lands at world (0,0,0). Useful right after importing assets that
    // shipped with random local offsets.
    // Slice 357 — 3ds Max / Blender vertex-snap completion. Given a
    // target world point, move the selected mesh so its CLOSEST vertex
    // lands exactly on the target. Pair with __studioFindSnapTarget
    // (slice 297) for snap-while-dragging gizmo workflows.
    // Slice 358 — Blender Symmetrize / Maya Mirror Geometry. Takes one
    // half of the selected mesh (per chosen axis + side) and mirrors
    // it across the plane to replace the other half. Direction:
    // '+x' = keep +X side, mirror to -X; '-x' = keep -X side. Same for
    // y/z. MVP modifies the position attribute in place.
    // Slice 359 — Maya / Blender "Group / Parent". Wraps the currently
    // selected meshes (or the active mesh if no multi-select) in a new
    // THREE.Group. The group is tagged so it shows up in the outliner;
    // children keep their world transforms via attach() so the visual
    // result is unchanged.
    // Slice 360 — Edit-mode primitive: pick the closest vertex of the
    // selected mesh to a world-space point. Returns the index + world
    // coords + distance. Foundation for click-to-select-vert in a
    // future interactive edit-mode (mouse-click → raycast → pick).
    // Slice 361 — Edit-mode primitive: translate a single vertex by a
    // world-space delta. Pairs with slice 360's __studioPickVertexAt for
    // a vert-drag interaction loop. Local-space delta is computed by
    // inverting the mesh.matrixWorld so the per-vert position attribute
    // stays in local coords.
    // Slice 362 — Blender Subdivide-edge / 3ds Max Connect. Inserts a
    // new vertex at the midpoint of the given edge, splitting one
    // triangle into two. MVP: works on non-indexed geometry (BoxGeometry
    // primitives ship non-indexed) by appending three new vertices
    // and re-emitting the two child triangles in place of the parent.
    // Slice 363 — Maya Combine / Blender Join. Merges N mesh uuids into a
    // single new mesh, baking each source's world transform into its
    // vertex positions before union. The new mesh inherits the first
    // source's material clone. Sources stay in scene (caller can delete).
    // Slice 364 — scene-stats diagnostic. Returns a compact snapshot:
    // primitive count + per-mesh {uuid, name, kind, verts, tris} table.
    // Handy for Archie self-checks and debugging dense scenes.
    // Slice 365 — Ctrl+A select-all-primitives. Replaces the multi-select
    // ref with every archdiscStudioPrimitive mesh; bumps outliner tick so
    // the selection-count badge updates.
    // Slice 366 — Blender Ctrl+I / Maya Invert Selection. Replaces the
    // multi-select set with every primitive NOT currently selected.
    // Slice 368 — Blender Numpad-. ("Frame Selected") / Maya F. Frames
    // the camera onto JUST the active mesh, not the whole scene. Useful
    // when one mesh is dwarfed by sprawling siblings.
    // Slice 370 — Houdini Mountain SOP. A high-poly sphere with fBm
    // noise displacement baked into vertex positions, producing a
    // crinkled rocky surface in one call. Common base mesh for
    // procedural terrain / asteroid / rock workflows.
    window.__studioAddMountain = (opts = {}) => {
      const scene = window.__archdiscScene; if (!scene) return { ok: false, error: 'no scene' };
      ...
      // Self-contained fBm hash so we don't depend on studioFbm3 closure.
      const hash = (x, y, z) => {
    ...
    window.__studioFitSelected = () => {
      const m = selectedMeshRef.current;
      const vp = window.__archdiscViewport;
      if (!m || !m.geometry || !vp || !vp.camera) return { ok: false, error: 'no mesh / no viewport' };
      m.updateMatrixWorld(true);
      if (!m.geometry.boundingSphere) m.geometry.computeBoundingSphere();
      const sphere = m.geometry.boundingSphere.clone().applyMatrix4(m.matrixWorld);
      const cam = vp.camera;
      const ctrl = vp.orbitControls || vp.controls;
      const fovY = (cam.fov || 45) * Math.PI / 180;
      const dist = sphere.radius / Math.sin(fovY / 2) * 1.4;
      // Keep camera direction; just move it along the look-at vector to the new distance.
      const dir = new THREE.Vector3().subVectors(cam.position, ctrl && ctrl.target ? ctrl.target : new THREE.Vector3()).normalize();
      const tgt = sphere.center;
      cam.position.copy(tgt).addScaledVector(dir, dist);
      if (ctrl && ctrl.target) { ctrl.target.copy(tgt); if (ctrl.update) ctrl.update(); }
      cam.lookAt(tgt);
      return { ok: true, center: [tgt.x, tgt.y, tgt.z], distance: dist, radius: sphere.radius };
    };
    window.__studioSelectInverse = () => {
      const scene = window.__archdiscScene; if (!scene) return { ok: false, error: 'no scene' };
      const all = [];
      scene.traverse((o) => {
        if (o.isMesh && o.userData && o.userData.archdiscStudioPrimitive) all.push(o);
      });
      const currentSet = new Set(
        (selectedMeshesRef.current || []).map((m) => m.uuid)
      );
      const inverse = all.filter((m) => !currentSet.has(m.uuid));
      if (!inverse.length) {
        // Everything was already selected — clear selection.
        selectedMeshesRef.current = [];
        if (window.__studioBumpOutliner) window.__studioBumpOutliner();
        return { ok: true, count: 0 };
      }
      if (window.__studioSelectMesh) window.__studioSelectMesh(inverse[inverse.length - 1]);
      selectedMeshesRef.current = inverse;
      if (window.__studioBumpOutliner) window.__studioBumpOutliner();
      return { ok: true, count: inverse.length };
    };
    window.__studioSelectAll = () => {
      const scene = window.__archdiscScene; if (!scene) return { ok: false, error: 'no scene' };
      const set = [];
      scene.traverse((o) => {
        if (o.isMesh && o.userData && o.userData.archdiscStudioPrimitive) set.push(o);
      });
      if (!set.length) return { ok: true, count: 0 };
      // __studioSelectMesh resets the multi-select set to [mesh] — call it
      // FIRST to attach the gizmo / set the active mesh, THEN overwrite the
      // multi-select ref with the full set.
      if (window.__studioSelectMesh) window.__studioSelectMesh(set[set.length - 1]);
      selectedMeshesRef.current = set;
      if (window.__studioBumpOutliner) window.__studioBumpOutliner();
    ...
    window.__studioListSceneStats = () => {
      ...
      scene.traverse((o) => {
        ...
        const t = g ? (g.index ? g.index.count / 3 : (g.attributes && g.attributes.position ? g.attributes.position.count / 3 : 0)) : 0;
      ...
      return { ok: true, count: meshes.length, totalVerts, totalTris: Math.round(totalTris), meshes };
    };
    window.__studioMergeMeshes = (uuids) => {
      const scene = window.__archdiscScene;
      if (!scene || !Array.isArray(uuids) || uuids.length < 2) return { ok: false, error: 'need >= 2 uuids' };
      const meshes = [];
      scene.traverse((o) => { if (o.isMesh && uuids.includes(o.uuid)) meshes.push(o); });
      if (meshes.length < 2) return { ok: false, error: 'fewer than 2 uuids resolved to meshes' };
      const baked = meshes.map((mesh) => {
        const g = (mesh.geometry.index ? mesh.geometry.toNonIndexed() : mesh.geometry.clone());
        mesh.updateMatrixWorld(true);
        g.applyMatrix4(mesh.matrixWorld);
        // Strip attributes that differ across sources so mergeGeometries works.
        for (const k of ['uv', 'uv1', 'uv2', 'tangent', 'color', 'skinIndex', 'skinWeight']) {
          if (g.attributes[k]) g.deleteAttribute(k);
        }
        return g;
      });
      const merged = mergeGeometries(baked, false);
      if (!merged) {
        baked.forEach((g) => g.dispose && g.dispose());
        return { ok: false, error: 'mergeGeometries returned null' };
      }
      merged.computeVertexNormals();
      merged.computeBoundingSphere();
      merged.computeBoundingBox();
      const mat = meshes[0].material.clone ? meshes[0].material.clone() : new THREE.MeshStandardMaterial({ color: 0x8a8f98 });
      const out = new THREE.Mesh(merged, mat);
      out.userData.archdiscStudioPrimitive = true;
      out.userData.archdiscStudioPrimitiveKind = 'merged';
      out.userData.archdiscStudioMergedFrom = uuids.slice();
      out.name = `studio-primitive-merged-${meshes.length}`;
      scene.add(out);
      baked.forEach((g) => g.dispose && g.dispose());
      if (primitiveStackRef.current) primitiveStackRef.current.push(out);
      setPrimitiveCount((c) => c + 1);
      return { ok: true, uuid: out.uuid, sourceCount: meshes.length, vertCount: merged.attributes.position.count };
    };
    window.__studioInsertVertexOnEdge = (meshUuid, edgeIdx) => {
      const scene = window.__archdiscScene; if (!scene) return { ok: false, error: 'no scene' };
      let m = selectedMeshRef.current;
      if (meshUuid) scene.traverse((o) => { if (o.isMesh && o.uuid === meshUuid) m = o; });
      if (!m || !m.geometry || !m.geometry.attributes.position) return { ok: false, error: 'no mesh' };
      // For indexed geometry, snapshot to non-indexed so the per-tri vert
      // duplication below has clean per-face positions to operate on.
      if (m.geometry.index) {
        const flat = m.geometry.toNonIndexed();
        if (m.geometry.dispose) m.geometry.dispose();
        m.geometry = flat;
      }
      const edges = window.__studioListEdges(m.uuid);
      ...
      // Find every triangle that contains both a and b — append a new vertex M
      // and split the triangle (a, b, c) into (a, m, c) and (m, b, c).
      ...
      for (let t = 0; t < triCount; t++) {
        ...
        const idxs = [i0, i1, i2];
        // Find positions matching a + b (positional match, since non-indexed verts
        // duplicate per face).
        ...
        // Append vertex M at end and rewrite this triangle's b → M, then add a
        // new triangle (M, b, c). To keep things simple we'll just push 3 new
        // verts per split (M+b'+c' duplicates).
        const cx = pos.getX(posC), cy = pos.getY(posC), cz = pos.getZ(posC);
        // Modify current triangle to (a, m, c).
    ...
    window.__studioMoveVertex = (meshUuid, vertIdx, deltaWorld) => {
      const scene = window.__archdiscScene; if (!scene) return { ok: false, error: 'no scene' };
      let m = selectedMeshRef.current;
      if (meshUuid) scene.traverse((o) => { if (o.isMesh && o.uuid === meshUuid) m = o; });
      if (!m || !m.geometry || !m.geometry.attributes.position) return { ok: false, error: 'no mesh' };
      ...
      m.updateMatrixWorld(true);
      // Convert world delta → local delta by applying inverse rotation/scale.
      // For translation-only deltas (no rotation/scale on the mesh) this is
      // identity; for rotated meshes we need to undo the matrix's R/S part.
      const inv = new THREE.Matrix4().copy(m.matrixWorld).invert();
      // Transform the delta as a DIRECTION, not a point — strip translation.
    ...
    // Slice 376 — Edit-mode state. Persists across re-renders via a global
    // ref. Mode is 'object' | 'vertex' | 'edge' | 'face' | 'sculpt'. Next
    // slice wires the canvas pointerdown to dispatch the right picker.
    if (!window.__studioEditModeRef) window.__studioEditModeRef = { current: 'object' };
    const _validEditModes = new Set(['object', 'vertex', 'edge', 'face', 'sculpt']);
    window.__studioSetEditMode = (mode) => {
      if (!_validEditModes.has(mode)) return { ok: false, error: 'invalid mode', valid: Array.from(_validEditModes) };
    ...
    // Slice 390 — Subdivide selected faces. Each selected triangle is
    // split into 4 sub-triangles via the 3 edge midpoints. Net per face:
    // +3 verts, +3 triangles. Blender W > Subdivide parity for the
    // triangle case.
    window.__studioSubdivideSelectedFaces = () => {
      const m = selectedMeshRef.current;
      if (!m || !m.geometry || !m.geometry.attributes.position) return { ok: false, error: 'no mesh' };
      ...
      for (let i = 0; i < posAttr.count; i++) {
        outVerts.push(posAttr.getX(i), posAttr.getY(i), posAttr.getZ(i));
      }
      for (let f = 0; f < triCount; f++) {
        if (selSet.has(f)) continue;
        outIdx.push(idx[f * 3], idx[f * 3 + 1], idx[f * 3 + 2]);
      }
      const vA = new THREE.Vector3(); const vB = new THREE.Vector3(); const vC = new THREE.Vector3();
      for (const f of sel.faces) {
        const ia = idx[f * 3]; const ib = idx[f * 3 + 1]; const ic = idx[f * 3 + 2];
        vA.fromBufferAttribute(posAttr, ia);
        vB.fromBufferAttribute(posAttr, ib);
        vC.fromBufferAttribute(posAttr, ic);
        const mABi = outVerts.length / 3;
        outVerts.push((vA.x + vB.x) / 2, (vA.y + vB.y) / 2, (vA.z + vB.z) / 2);
        const mBCi = mABi + 1;
        outVerts.push((vB.x + vC.x) / 2, (vB.y + vC.y) / 2, (vB.z + vC.z) / 2);
        const mCAi = mABi + 2;
        outVerts.push((vC.x + vA.x) / 2, (vC.y + vA.y) / 2, (vC.z + vA.z) / 2);
        // 4 sub-triangles maintaining the original winding.
        outIdx.push(ia, mABi, mCAi);
        outIdx.push(mABi, ib, mBCi);
        outIdx.push(mCAi, mBCi, ic);
        outIdx.push(mABi, mBCi, mCAi);
      }
      ...
      if (window.__studioRenderEditSelectionMarkers) window.__studioRenderEditSelectionMarkers();
      return { ok: true, addedTris: sel.faces.length * 3, vertCount: newGeo.attributes.position.count, triCount: Math.floor(newGeo.index.array.length / 3) };
    };
    // Slice 389 — Inset selected faces. For each selected triangle:
    // - 3 new verts at lerp(orig, centroid, t) — pulled toward face centre
    // - cap triangle from the 3 new verts (same winding)
    // - 6 wall triangles connecting old edges to inset edges
    // factor t in (0, 1); 0 = no inset, 1 = collapse to centroid.
    // Blender I-hotkey parity for the simple triangle case.
    window.__studioInsetSelectedFaces = (factor) => {
      const m = selectedMeshRef.current;
      if (!m || !m.geometry || !m.geometry.attributes.position) return { ok: false, error: 'no mesh' };
      if (typeof factor !== 'number' || !Number.isFinite(factor) || factor <= 0 || factor >= 1) return { ok: false, error: 'bad factor (need 0<t<1)' };
      const sel = window.__studioEditSelection.current;
      if (!sel.faces.length) return { ok: false, error: 'no faces selected' };
      const geo = m.geometry;
      const posAttr = geo.attributes.position;
      ...
      for (const f of sel.faces) {
        ...
        const lerp = (v, cX, cY, cZ) => [
          v.x + (cX - v.x) * factor,
          v.y + (cY - v.y) * factor,
          v.z + (cZ - v.z) * factor,
        ];
        const nA = lerp(vA, cx, cy, cz);
        const nB = lerp(vB, cx, cy, cz);
        const nC = lerp(vC, cx, cy, cz);
        const newAi = outVerts.length / 3; outVerts.push(...nA);
        const newBi = newAi + 1;           outVerts.push(...nB);
        const newCi = newAi + 2;           outVerts.push(...nC);
        // Inset cap.
        outIdx.push(newAi, newBi, newCi);
        // 3 connecting quads (6 tris) — each old edge → inset edge.
        outIdx.push(ia, ib, newBi);
        outIdx.push(ia, newBi, newAi);
        outIdx.push(ib, ic, newCi);
        outIdx.push(ib, newCi, newBi);
        outIdx.push(ic, ia, newAi);
        outIdx.push(ic, newAi, newCi);
      }
      const newGeo = new THREE.BufferGeometry();
      newGeo.setAttribute('position', new THREE.Float32BufferAttribute(outVerts, 3));
      newGeo.setIndex(outIdx);
      ...
      window.__studioClearEditSelection();
      if (window.__studioRenderEditSelectionMarkers) window.__studioRenderEditSelectionMarkers();
      return { ok: true, addedTris: sel.faces.length * 7, vertCount: newGeo.attributes.position.count, triCount: Math.floor(newGeo.index.array.length / 3), factor };
    };
    // Slice 388 — True extrude on selected faces. Each selected triangle's
    // 3 verts are duplicated at (orig + normal * dist), a new cap triangle
    // is added in the same winding, and 3 side quads (6 tris) wall the
    // sides. The original triangle is removed. Non-selected triangles are
    // preserved verbatim. Rebuilds the whole BufferGeometry; clears the
    // edit selection so the user can re-pick the new cap.
    window.__studioExtrudeSelectedFaces = (dist) => {
      const m = selectedMeshRef.current;
      if (!m || !m.geometry || !m.geometry.attributes.position) return { ok: false, error: 'no mesh' };
      ...
      for (const f of sel.faces) {
        ...
        ab.subVectors(vB, vA);
        ac.subVectors(vC, vA);
        n.crossVectors(ab, ac).normalize().multiplyScalar(dist);
        const newAi = outVerts.length / 3;
        outVerts.push(vA.x + n.x, vA.y + n.y, vA.z + n.z);
        const newBi = newAi + 1;
        outVerts.push(vB.x + n.x, vB.y + n.y, vB.z + n.z);
        const newCi = newAi + 2;
        outVerts.push(vC.x + n.x, vC.y + n.y, vC.z + n.z);
        // Cap (new top face) — same winding as original.
        outIdx.push(newAi, newBi, newCi);
        // Side walls — each old edge → 2 triangles connecting old + new.
        // Edge a→b walls: (ia, ib, newBi) + (ia, newBi, newAi).
        outIdx.push(ia, ib, newBi);
        outIdx.push(ia, newBi, newAi);
        // Edge b→c.
        outIdx.push(ib, ic, newCi);
      ...
      m.geometry.dispose();
      m.geometry = newGeo;
      // Clear edit-mode selection — old face indices are now stale.
      window.__studioClearEditSelection();
      if (window.__studioRenderEditSelectionMarkers) window.__studioRenderEditSelectionMarkers();
      return { ok: true, addedTris: sel.faces.length * 7, vertCount: newGeo.attributes.position.count, triCount: Math.floor(newGeo.index.array.length / 3) };
    };
    // Slice 387 — Select every primitive of the given mode in the active
    // mesh (Blender Ctrl+A inside edit mode parity). Replaces the prior
    // selection. Re-renders markers.
    window.__studioSelectAllEdit = (mode) => {
      const m = selectedMeshRef.current;
      if (!m || !m.geometry || !m.geometry.attributes.position) return { ok: false, error: 'no mesh' };
      const pos = m.geometry.attributes.position;
      const idx = m.geometry.index ? m.geometry.index.array : null;
      const empty = { vertices: [], edges: [], faces: [] };
      window.__studioEditSelection.current = empty;
      if (mode === 'vertex') {
        const out = [];
        ...
        const out = [];
        for (let f = 0; f < triCount; f++) out.push(f);
        ...
        for (let f = 0; f < triCount; f++) {
          ...
          allKeys.add(a < b ? `${a}_${b}` : `${b}_${a}`);
          allKeys.add(b < c ? `${b}_${c}` : `${c}_${b}`);
          allKeys.add(c < a ? `${c}_${a}` : `${a}_${c}`);
        }
        const out = [];
        for (const k of allKeys) { const [a, b] = k.split('_').map(Number); out.push([a, b]); }
    ...
    // Slice 386 — Invert the current edit selection for a given mode. For
    // vertex/face: every index NOT in the current set becomes selected.
    // For edge: every unique mesh edge (the 12 cube edges, not the
    // triangulation diagonals) not already in the set. Re-renders markers
    // when done. Blender Ctrl+I parity inside edit mode.
    window.__studioInvertEditSelection = (mode) => {
      const m = selectedMeshRef.current;
      if (!m || !m.geometry || !m.geometry.attributes.position) return { ok: false, error: 'no mesh' };
      const pos = m.geometry.attributes.position;
      const idx = m.geometry.index ? m.geometry.index.array : null;
      const sel = window.__studioEditSelection.current;
      if (mode === 'vertex') {
        const have = new Set(sel.vertices);
        const out = [];
        for (let i = 0; i < pos.count; i++) if (!have.has(i)) out.push(i);
        window.__studioEditSelection.current.vertices = out;
      } else if (mode === 'face') {
        const have = new Set(sel.faces);
        const triCount = idx ? Math.floor(idx.length / 3) : Math.floor(pos.count / 3);
        const out = [];
    ...
    // Slice 382 — Move every vertex in the multi-selection by a world-space
    // delta. Edges contribute both endpoints; faces contribute all three
    // triangle vertices. Dedup'd via a Set so a vert shared between a
    // selected edge AND a selected face moves once, not twice. Re-renders
    // markers + pushes undo. The real edit op behind slice 381's selection.
    window.__studioMoveSelectedVerts = (dx, dy, dz) => {
      const m = selectedMeshRef.current;
      if (!m || !m.geometry || !m.geometry.attributes.position) return { ok: false, error: 'no mesh' };
      const sel = window.__studioEditSelection.current;
      const pos = m.geometry.attributes.position;
      ...
      if (window.__studioPushUndo) window.__studioPushUndo();
      // Convert world delta to local (mesh space) so it survives rotation /
      // scale on the parent. inverseMatrixWorld * worldDelta = localDelta.
      m.updateMatrixWorld(true);
      const inv = new THREE.Matrix4().copy(m.matrixWorld).invert();
      ...
      if (window.__studioRenderEditSelectionMarkers) window.__studioRenderEditSelectionMarkers();
      return { ok: true, vertCount: touched.size, deltaWorld: [dx, dy, dz], deltaLocal: [localDelta.x, localDelta.y, localDelta.z] };
    };
    // Slice 384 — Scale every vertex in the multi-selection around its
    // centroid by a uniform factor. Same dedup logic as MoveSelectedVerts.
    // factor=1 is a no-op; factor=0 collapses to the centroid; negative
    // factors mirror. Real edit op behind G's sibling S / Maya R hotkey.
    window.__studioScaleSelectedVerts = (factor) => {
      const m = selectedMeshRef.current;
      if (!m || !m.geometry || !m.geometry.attributes.position) return { ok: false, error: 'no mesh' };
      ...
      if (!touched.size) return { ok: false, error: 'empty selection' };
      if (window.__studioPushUndo) window.__studioPushUndo();
      // Centroid in LOCAL space — all verts live there anyway.
      let cx = 0, cy = 0, cz = 0;
      for (const v of touched) { cx += pos.getX(v); cy += pos.getY(v); cz += pos.getZ(v); }
      cx /= touched.size; cy /= touched.size; cz /= touched.size;
      for (const v of touched) {
        const x = cx + (pos.getX(v) - cx) * factor;
        const y = cy + (pos.getY(v) - cy) * factor;
        const z = cz + (pos.getZ(v) - cz) * factor;
        pos.setXYZ(v, x, y, z);
      }
      pos.needsUpdate = true;
      if (m.geometry.computeVertexNormals) m.geometry.computeVertexNormals();
    ...
    // Slice 385 — Rotate every vertex in the multi-selection around its
    // centroid by `angleRad` (radians) on the given axis ('x'|'y'|'z' or
    // a [x,y,z] vector). Same dedup'd set as 382/384. Completes the G/R/S
    // edit-op triad.
    window.__studioRotateSelectedVerts = (angleRad, axis) => {
      const m = selectedMeshRef.current;
      if (!m || !m.geometry || !m.geometry.attributes.position) return { ok: false, error: 'no mesh' };
    ...
    // user actually SEES what was selected. Old markers are removed
    // before each new pick so only one is visible at a time.
    const MARKER_GROUP = '__studio_pick_marker__';
    const _clearMarkers = () => {
      const scene = window.__archdiscScene;
      if (!scene) return;
      const toRemove = [];
      scene.traverse((o) => { if (o.name === MARKER_GROUP) toRemove.push(o); });
    ...
    // hits, then returns whichever of its 3 edges is closest to the hit
    // point. Completes the vert/face/edge edit-mode trio.
    window.__studioPickEdgeFromClick = (ndcX, ndcY) => {
      const vp = window.__archdiscViewport;
      const m = selectedMeshRef.current;
      if (!vp || !vp.camera || !m || !m.geometry || !m.geometry.attributes.position) return { ok: false, error: 'no mesh / no viewport' };
      const cam = vp.camera;
      cam.updateMatrixWorld(true);
      if (cam.matrixWorldInverse) cam.matrixWorldInverse.copy(cam.matrixWorld).invert();
      m.updateMatrixWorld(true);
      if (m.geometry.computeBoundsTree && !m.geometry.boundsTree) m.geometry.computeBoundsTree();
      ...
      const pos = m.geometry.attributes.position;
      const idx = m.geometry.index ? m.geometry.index.array : null;
      const faceIdx = hit.faceIndex == null ? 0 : hit.faceIndex;
      const ia = idx ? idx[faceIdx * 3] : faceIdx * 3;
      const ib = idx ? idx[faceIdx * 3 + 1] : faceIdx * 3 + 1;
      const ic = idx ? idx[faceIdx * 3 + 2] : faceIdx * 3 + 2;
      ...
      const distToSegment = (p, a, b) => {
        ...
        return { d: closest.distanceTo(p), point: closest, t };
      };
      const edges = [
    ...
    // Slice 374 — Click-pick face from NDC. Returns the hit triangle's
    // index plus its 3 vertex indices and world-space centroid — pairs
    // with Slice 373's vert pick for true edit-mode face select.
    window.__studioPickFaceFromClick = (ndcX, ndcY) => {
      const vp = window.__archdiscViewport;
      const m = selectedMeshRef.current;
      if (!vp || !vp.camera || !m || !m.geometry || !m.geometry.attributes.position) return { ok: false, error: 'no mesh / no viewport' };
      const cam = vp.camera;
      ...
      const idx = m.geometry.index ? m.geometry.index.array : null;
      const faceIdx = hit.faceIndex == null ? 0 : hit.faceIndex;
      const a = idx ? idx[faceIdx * 3] : faceIdx * 3;
      const b = idx ? idx[faceIdx * 3 + 1] : faceIdx * 3 + 1;
      const c = idx ? idx[faceIdx * 3 + 2] : faceIdx * 3 + 2;
      const vA = new THREE.Vector3().fromBufferAttribute(pos, a).applyMatrix4(m.matrixWorld);
      const vB = new THREE.Vector3().fromBufferAttribute(pos, b).applyMatrix4(m.matrixWorld);
      const vC = new THREE.Vector3().fromBufferAttribute(pos, c).applyMatrix4(m.matrixWorld);
      const cx = (vA.x + vB.x + vC.x) / 3;
    ...
    // Slice 373 — Click-pick interactive vert select. Casts a ray from the
    // current pointer NDC into the selected mesh and picks the closest
    // vertex along the ray's nearest hit. Foundation for true edit-mode.
    window.__studioPickVertexFromClick = (ndcX, ndcY) => {
      const vp = window.__archdiscViewport;
      const m = selectedMeshRef.current;
      if (!vp || !vp.camera || !m || !m.geometry || !m.geometry.attributes.position) return { ok: false, error: 'no mesh / no viewport' };
      const cam = vp.camera;
      ...
      if (cam.matrixWorldInverse) cam.matrixWorldInverse.copy(cam.matrixWorld).invert();
      m.updateMatrixWorld(true);
      // BVH-accel raycast needs boundsTree built on geometry.
      if (m.geometry.computeBoundsTree && !m.geometry.boundsTree) m.geometry.computeBoundsTree();
      const rc = new THREE.Raycaster();
      rc.setFromCamera(new THREE.Vector2(ndcX, ndcY), cam);
      rc.firstHitOnly = true;
      // three-mesh-bvh patches intersectObjects (plural), not intersectObject.
      const hits = rc.intersectObjects([m], false);
      if (!hits.length) return { ok: false, error: 'no hit' };
      const hit = hits[0];
      const pos = m.geometry.attributes.position;
      ...
      return {
        ok: true, vertIdx: pickIdx, faceIdx,
        point: [pickWorld.x, pickWorld.y, pickWorld.z],
        distance: pickDist,
      };
    };
    window.__studioPickVertexAt = (worldPoint) => {
      const m = selectedMeshRef.current;
      if (!m || !m.geometry || !m.geometry.attributes.position) return { ok: false, error: 'no mesh' };
    ...
    window.__studioGroupSelected = (name = 'group') => {
      const scene = window.__archdiscScene;
      if (!scene) return { ok: false, error: 'no scene' };
      const set = (selectedMeshesRef.current && selectedMeshesRef.current.length)
        ? selectedMeshesRef.current.slice()
        : (selectedMeshRef.current ? [selectedMeshRef.current] : []);
      if (!set.length) return { ok: false, error: 'no selection' };
      const group = new THREE.Group();
      group.name = `studio-group-${name}`;
      group.userData.archdiscStudioGroup = true;
      group.userData.archdiscStudioGroupName = String(name);
      scene.add(group);
      for (const m of set) {
        // attach() preserves world transform when moving to a new parent.
        group.attach(m);
      }
      if (primitiveStackRef.current) primitiveStackRef.current.push(group);
      return { ok: true, uuid: group.uuid, name: group.name, children: set.length };
    };
    window.__studioSymmetrize = (direction = '+x') => {
      const m = selectedMeshRef.current;
      if (!m || !m.geometry || !m.geometry.attributes.position) return { ok: false, error: 'no mesh' };
      const d = String(direction).toLowerCase();
      const valid = ['+x', '-x', '+y', '-y', '+z', '-z'];
      if (!valid.includes(d)) return { ok: false, error: 'direction must be ±x/y/z' };
      const axis = d[1];
      const keepPositive = d[0] === '+';
      const pos = m.geometry.attributes.position;
      let flipped = 0;
      for (let i = 0; i < pos.count; i++) {
        const v = (axis === 'x' ? pos.getX(i) : axis === 'y' ? pos.getY(i) : pos.getZ(i));
        const shouldFlip = keepPositive ? (v < 0) : (v > 0);
        if (shouldFlip) {
      ...
      m.geometry.computeBoundingSphere();
      m.geometry.computeBoundingBox();
      m.userData.archdiscStudioSymmetrized = (m.userData.archdiscStudioSymmetrized || 0) + 1;
      return { ok: true, direction: d, axis, keepPositive, flipped };
    };
    window.__studioSnapToVertex = (targetWorld) => {
      const m = selectedMeshRef.current;
      if (!m || !m.geometry || !m.geometry.attributes.position) return { ok: false, error: 'no mesh' };
      if (!Array.isArray(targetWorld) || targetWorld.length !== 3) return { ok: false, error: 'targetWorld must be [x,y,z]' };
      const t = new THREE.Vector3(targetWorld[0], targetWorld[1], targetWorld[2]);
      const pos = m.geometry.attributes.position;
      m.updateMatrixWorld(true);
      const v = new THREE.Vector3();
      let best = Infinity, bestIdx = -1;
      const bestVert = new THREE.Vector3();
      for (let i = 0; i < pos.count; i++) {
        v.fromBufferAttribute(pos, i).applyMatrix4(m.matrixWorld);
        const d = v.distanceTo(t);
        if (d < best) { best = d; bestIdx = i; bestVert.copy(v); }
      }
      const dx = t.x - bestVert.x, dy = t.y - bestVert.y, dz = t.z - bestVert.z;
      m.position.x += dx; m.position.y += dy; m.position.z += dz;
      m.updateMatrixWorld(true);
      m.userData.archdiscStudioSnappedToVertex = (m.userData.archdiscStudioSnappedToVertex || 0) + 1;
      return { ok: true, snappedIdx: bestIdx, distance: best, offset: [dx, dy, dz] };
    };
    window.__studioCenterAtOrigin = () => {
      const m = selectedMeshRef.current;
      if (!m || !m.geometry) return { ok: false, error: 'no mesh' };
      m.updateMatrixWorld(true);
      if (!m.geometry.boundingBox) m.geometry.computeBoundingBox();
      ...
      m.position.z -= cz;
      m.updateMatrixWorld(true);
      m.userData.archdiscStudioCenteredAtOrigin = (m.userData.archdiscStudioCenteredAtOrigin || 0) + 1;
      return { ok: true, offset: [cx, cy, cz], newPos: [m.position.x, m.position.y, m.position.z] };
    };
    window.__studioRandomScatter = (opts = {}) => {
      const m = selectedMeshRef.current;
      const scene = window.__archdiscScene;
      if (!scene || !m || !m.geometry || !m.material) return { ok: false, error: 'no mesh' };
      const { count = 8, radius = 0.1, seed = 1, jitterRot = true } = opts;
      ...
      const created = [];
      for (let i = 1; i <= n; i++) {
        // Uniform random point in sphere via rejection-sampling-friendly trig.
        ...
        clone.material = m.material.clone();
        clone.position.set(m.position.x + dx, m.position.y + dy, m.position.z + dz);
        if (jitterRot) clone.rotation.set(rng() * Math.PI * 2, rng() * Math.PI * 2, rng() * Math.PI * 2);
        clone.userData.archdiscStudioPrimitive = true;
        clone.userData.archdiscStudioPrimitiveKind = (m.userData && m.userData.archdiscStudioPrimitiveKind) || 'scatter-clone';
        clone.userData.archdiscStudioScatteredFrom = m.uuid;
        clone.name = `${m.name || 'studio-primitive'}-scatter-${i}`;
        scene.add(clone);
        if (primitiveStackRef.current) primitiveStackRef.current.push(clone);
        created.push(clone.uuid);
      }
      setPrimitiveCount((c) => c + n);
      return { ok: true, count: n, radius, seed, created };
    };
    window.__studioCloneAlongAxis = (opts = {}) => {
      const m = selectedMeshRef.current;
      const scene = window.__archdiscScene;
      if (!scene || !m || !m.geometry || !m.material) return { ok: false, error: 'no mesh' };
      const { count = 3, axis = 'x', spacing = 0.05 } = opts;
      const a = (axis || 'x').toLowerCase();
      if (a !== 'x' && a !== 'y' && a !== 'z') return { ok: false, error: 'axis must be x/y/z' };
      const n = Math.max(1, count | 0);
      const created = [];
      for (let i = 1; i <= n; i++) {
        const clone = m.clone();
        clone.geometry = m.geometry; // share geometry; cheap
        clone.material = m.material.clone();
        clone.position[a] = m.position[a] + i * spacing;
        clone.userData.archdiscStudioPrimitive = true;
        clone.userData.archdiscStudioPrimitiveKind = (m.userData && m.userData.archdiscStudioPrimitiveKind) || 'array-clone';
        clone.userData.archdiscStudioClonedFrom = m.uuid;
        clone.name = `${m.name || 'studio-primitive'}-clone-${a}${i}`;
        scene.add(clone);
        if (primitiveStackRef.current) primitiveStackRef.current.push(clone);
        created.push(clone.uuid);
      }
      setPrimitiveCount((c) => c + n);
      return { ok: true, count: n, axis: a, spacing, created };
    };
    window.__studioAlignToGround = (opts = {}) => {
      const m = selectedMeshRef.current;
      if (!m || !m.geometry) return { ok: false, error: 'no mesh' };
      ...
      m.position.y += dy;
      m.updateMatrixWorld(true);
      m.userData.archdiscStudioAlignedToGround = (m.userData.archdiscStudioAlignedToGround || 0) + 1;
      return { ok: true, floor, dy, newY: m.position.y };
    };
    window.__studioLookAt = (target) => {
      const m = selectedMeshRef.current;
      if (!m) return { ok: false, error: 'no mesh selected' };
      let tx = 0, ty = 0, tz = 0;
      if (Array.isArray(target)) {
        ...
        const scene = window.__archdiscScene; if (!scene) return { ok: false, error: 'no scene' };
        let to = null;
        scene.traverse((o) => { if (o.isMesh && o.uuid === target) to = o; });
        if (!to) return { ok: false, error: 'target uuid not found' };
        to.updateMatrixWorld(true);
        tx = to.position.x; ty = to.position.y; tz = to.position.z;
      } else {
        return { ok: false, error: 'target must be [x,y,z] or mesh uuid' };
      }
      m.lookAt(tx, ty, tz);
      m.updateMatrixWorld(true);
      // NOTE: intentionally NOT calling setSelectedTransform here — the
      // resulting React render was racing the test's next evaluate and
      // briefly nuking window.__studioLookAt. N-panel inputs lag by one
      // gizmo tick; not worth the race.
      m.userData.archdiscStudioLookAt = (m.userData.archdiscStudioLookAt || 0) + 1;
      return { ok: true, target: [tx, ty, tz], rotation: [m.rotation.x, m.rotation.y, m.rotation.z] };
    };
    window.__studioMarkEdgeSeam = (meshUuid, edgeIdx, marked = true) => {
      const scene = window.__archdiscScene;
      if (!scene) return { ok: false, error: 'no scene' };
      let mesh = selectedMeshRef.current;
      if (meshUuid) scene.traverse((o) => { if (o.isMesh && o.uuid === meshUuid) mesh = o; });
      if (!mesh || !mesh.geometry) return { ok: false, error: 'no mesh' };
      const edges = window.__studioListEdges(mesh.uuid);
      const e = edges[edgeIdx | 0];
      if (!e) return { ok: false, error: 'edge idx out of range' };
    ...
    window.__studioListSeams = (meshUuid) => {
    ...
    window.__studioClearSeams = (meshUuid) => {
    ...
    window.__studioListEdges = (meshUuid) => {
      const scene = window.__archdiscScene; if (!scene) return [];
      let mesh = selectedMeshRef.current;
      if (meshUuid) scene.traverse((o) => { if (o.isMesh && o.uuid === meshUuid) mesh = o; });
    ...
    window.__studioPickEdge = (meshUuid, edgeIdx) => {
      const scene = window.__archdiscScene; if (!scene) return { ok: false, error: 'no scene' };
      let mesh = selectedMeshRef.current;
      if (meshUuid) scene.traverse((o) => { if (o.isMesh && o.uuid === meshUuid) mesh = o; });
      if (!mesh || !mesh.geometry) return { ok: false, error: 'no mesh' };
    ...
    window.__studioClearEdgeSelection = () => {
      const h = window.__studioPickedEdgeHelper;
    ...
    // Slice 297 — 3ds Max-style vertex snap target finder. Returns the
    // nearest vertex in the scene to fromPos, excluding the selected mesh
    // (so the gizmo doesn't snap back to its own pivot). MVP: vertex only;
    // edge/face are deferred.
    // Slice 299 — Cinema 4D / MagicaVoxel voxelization MVP. Triangle-
    // rasterises a mesh into a resolution³ occupancy grid by barycentric
    // oversampling. Returns the grid + voxel count + bbox so callers can
    // feed marching cubes (deferred to a followup slice).
    // Slice 306 — Cinema 4D Volume Mesher: marching-cubes extraction from a
    // voxel occupancy grid (slice 299 output) into a fresh BufferGeometry,
    // then add a Mesh to the scene. The result reads as a smooth blob —
    // exactly Cinema 4D's "Volume Builder → Volume Mesher" pair.
    // Slice 323 — Houdini OpenVDB / C4D Volume Builder parity. Ray-marched
    // fog cube via a custom ShaderMaterial. The fragment shader steps inside
    // the cube and accumulates fbm-noise density. Camera-local raymarch via
    // onBeforeRender so it follows orbit.
    window.__studioAddVolume = (opts = {}) => {
      const scene = window.__archdiscScene;
      if (!scene) return { ok: false, error: 'no scene' };
    ...
    window.__studioMarchVoxelGrid = (opts) => {
      const scene = window.__archdiscScene; if (!scene) return { ok: false, error: 'no scene' };
      ...
      for (let n = 0; n < vCount * 3; n += 3) {
        // MarchingCubes lives in [-1, 1]; remap to bbox world coords.
      ...
      scene.add(outMesh);
      if (primitiveStackRef.current) primitiveStackRef.current.push(outMesh);
      setPrimitiveCount((c) => c + 1);
      return { ok: true, uuid: outMesh.uuid, vertices: vCount, resolution: res };
    };
    window.__studioVoxelizeMesh = (opts) => {
      const scene = window.__archdiscScene;
      if (!scene) return { ok: false, error: 'no scene' };
      ...
      const stepDen = res * 2; // oversample 2x to avoid holes
    ...
    // Slice 314 — BVH-accelerated raycasting (three-mesh-bvh). Lazily builds
    // a bounds tree per mesh on first query, invalidates when the position
    // attribute version bumps (Three auto-bumps on needsUpdate). Returns the
    // closest hit or {hit:false}. filterUuid excludes one mesh (e.g. the
    // currently-selected mesh during a snap-from-gizmo flow).
    ...
    window.__studioRaycastBVH = (opts) => {
      const { origin, direction, far = Infinity, filterUuid = null, firstHitOnly = true } = (opts || {});
      const scene = window.__archdiscScene;
      if (!scene || !origin || !direction) return { hit: false, error: 'bad args' };
      const meshes = [];
      scene.traverse((o) => {
        if (!o.isMesh || !o.userData || !o.userData.archdiscStudioPrimitive) return;
      ...
      if (!hits.length) return { hit: false, point: null, meshUuid: null, distance: Infinity, faceIndex: -1, bvh: __BVH_OK };
      const h = hits[0];
      return {
        hit: true,
    ...
    window.__studioFindSnapTarget = (opts) => {
      const { fromPos, kinds = ['vertex'], radius = 0.5 } = (opts || {});
      const scene = window.__archdiscScene;
      if (!scene || !fromPos) return null;
      if (!kinds.includes('vertex')) return null;
      const from = new THREE.Vector3(fromPos[0], fromPos[1], fromPos[2]);
      const selUuid = selectedMeshRef.current && selectedMeshRef.current.uuid;
      let best = null;
      const tmp = new THREE.Vector3();
      scene.traverse((o) => {
    ...
    window.__studioBakeAOToTexture = (size, rays) => bakeAOToTextureOp(size || 256, rays);
    // Slice 746 — Substance Painter / Marmoset style UV-baked maps for
    // AO + tangent-space normal + world-space position. The slice-284
    // `bakeAOToTexture` was already a real per-texel hemisphere ray-trace,
    // so __studioBakeAOMap is its alias; normal + position siblings ship
    // here. Auto-create uv1 for the AO bake so MeshStandardMaterial's
    // `aoMap` slot reads correctly.
    window.__studioBakeAOMap = (size, rays) => bakeAOToTextureOp(size || 256, rays);
    window.__studioBakeNormalMap = (size) => {
      const m = selectedMeshRef.current;
      if (!m) return { ok: false, error: 'no mesh selected' };
      return bakeNormalToTexture(m, { size: size || 256 });
    };
    window.__studioBakePositionMap = (size) => {
      const m = selectedMeshRef.current;
      if (!m) return { ok: false, error: 'no mesh selected' };
      return bakePositionToTexture(m, { size: size || 256 });
    };
    // Slice 295 — Mari project-paint from current camera view.
    window.__studioProjectPaintFromCamera = (opts) => {
      const m = selectedMeshRef.current;
      const vp = window.__archdiscViewport;
      if (!m || !vp || !vp.camera) return { ok: false, error: 'no mesh or camera' };
      if (!opts || !opts.sourceImageData) return { ok: false, error: 'no sourceImageData' };
      return projectPaintFromCamera(m, vp.camera, opts.sourceImageData, { size: opts.size || 256 });
    };
    // Slice 308 — Substance Painter "Smart Material" preset. One call sets
    // up a believable PBR look: perlin map for color, voronoi for roughness,
    // and a fast AO bake to material.aoMap.
    window.__studioApplySmartMaterial = (preset) => {
      const m = selectedMeshRef.current;
      if (!m || !m.material) return { ok: false, error: 'no mesh selected' };
      const PRESETS = {
        rust:     { c1: '#2a1308', c2: '#a04518', rough: ['#404040', '#e8e8e8'], freq: 6, cells: 14, met: 0.65 },
        concrete: { c1: '#a8a8a4', c2: '#6e6e68', rough: ['#888888', '#e0e0e0'], freq: 5, cells: 22, met: 0.0  },
        stone:    { c1: '#4d4540', c2: '#bfb6a5', rough: ['#666666', '#cfcfcf'], freq: 4, cells: 10, met: 0.05 },
      };
      const p = PRESETS[preset];
      if (!p) return { ok: false, error: 'unknown preset: ' + preset };
      const mapR = window.__studioProceduralTexture && window.__studioProceduralTexture({
        type: 'perlin', size: 256, channel: 'map',
        params: { freq: p.freq, octaves: 5, color1: p.c1, color2: p.c2, seed: 42 },
      });
      const roughR = window.__studioProceduralTexture && window.__studioProceduralTexture({
        type: 'voronoi', size: 128, channel: 'roughnessMap',
        params: { freq: 2, cells: p.cells, color1: p.rough[0], color2: p.rough[1], seed: 11 },
      });
      const aoR = window.__studioBakeAOToTexture && window.__studioBakeAOToTexture(64, 8);
      m.material.metalness = p.met;
      m.material.needsUpdate = true;
      m.userData.archdiscStudioSmartMat = preset;
      return { ok: true, preset, map: !!(mapR && mapR.ok), rough: !!(roughR && roughR.ok), ao: !!(aoR && aoR.ok) };
    };
    // Slice 294 — Substance Designer procedural noise → UV texture.
    window.__studioProceduralTexture = (opts) => {
      const m = selectedMeshRef.current;
      if (!m) return { ok: false, error: 'no mesh selected' };
      return generateProceduralTexture(m, opts || {});
    };
  ...
  if (typeof document !== 'undefined' && !blenderKeymapRef.current.installed) {
    ...
    document.addEventListener('keydown', (e) => {
      ...
      if (k === 'g' && e.altKey && set.length) {
        ...
        // Utils mergeVertices). Real kernel parity (Blender Mesh >
        // Clean Up > Merge by Distance, Maya Edit Mesh > Merge).
        if (window.__studioPushUndo) window.__studioPushUndo();
        if (mesh.geometry) {
          try {
  ...
  // Slice 293 — Plasticity/MoI fillet-edges MVP. Welds verts via
  // mergeVertices, builds an edge→face[2] map, computes the dihedral
  // angle of every manifold edge, and for vertices that touch any sharp
  // edge (dihedral > threshold) shifts them inward by `radius` along the
  // averaged negative face normal. After computeVertexNormals the result
  // reads as a chamfered/filleted hard edge — same MoI preview-LOD trick.
  function filletEdges({ meshUuid, radius = 0.05, threshold = Math.PI / 6 } = {}) {
    ...
    // Strip non-position attrs so mergeVertices can weld; recompute after.
    ...
    // Edge map: "minIdx,maxIdx" → [triA, triB]. Vertex → adjacent triangles.
    const edgeMap = new Map();
    ...
    // Mark verts that touch any sharp edge.
    ...
    // Shift sharp verts inward by `radius` along the averaged face normal
    // (negated so the vert moves into the body).
    ...
    recomputeMeshStats(scene);
    return { meshUuid: mesh.uuid, filletedEdges: sharpEdges, shiftedVerts: sharpVerts.size, radius, threshold };
  }
  ...
  // Slice 284 — Substance Painter / Mari workflow: bake AO to a UV-mapped
  // texture map (not vertex colours). Wire it as mesh.material.aoMap.
  function bakeAOToTextureOp(size, raysOverride) {
    const mesh = selectedMeshRef.current;
    if (!mesh || !mesh.geometry) return null;
    const scene = window.__archdiscScene;
    const occluders = [];
    if (scene) scene.traverse((o) => { if (o.isMesh && o.userData && o.userData.archdiscStudioPrimitive) occluders.push(o); });
    const r = bakeAOToTexture(mesh, occluders.length ? occluders : [mesh], { size, rays: raysOverride });
    if (r && r.ok) mesh.userData.archdiscStudioBakedTexture = 'ao';
    return r;
  }
              ...
              <>
                ...
                <div className="ribbon-group">
                  <div className="ribbon-group-tools">
                    ...
                    <button type="button" className="ribbon-tool" data-studio-ribbon-action="bake-ao-map" onClick={() => window.__studioBakeAOMap && window.__studioBakeAOMap(256)} disabled={!selectedKind} title="Substance Painter / Marmoset — UV-baked AO texture (hemisphere ray-trace into material.aoMap). Real per-texel bake, not vertex colours.">
                      <span className="ribbon-tool-icon">⊕</span><span className="ribbon-tool-label">AO Map</span>
                    </button>
                    <button type="button" className="ribbon-tool" data-studio-ribbon-action="bake-normal-map" onClick={() => window.__studioBakeNormalMap && window.__studioBakeNormalMap(256)} disabled={!selectedKind} title="Substance Painter / Marmoset — UV-baked tangent-space normal map (n*0.5+0.5 RGB into material.normalMap). Real per-texel bake, not vertex colours.">
                      <span className="ribbon-tool-icon">↥</span><span className="ribbon-tool-label">N Map</span>
                    </button>
                    <button type="button" className="ribbon-tool" data-studio-ribbon-action="bake-position-map" onClick={() => window.__studioBakePositionMap && window.__studioBakePositionMap(256)} disabled={!selectedKind} title="Substance Painter / Marmoset — UV-baked world-space position map (RGB = normalized bbox coords) into userData.archdiscStudioPositionMap. Real per-texel bake, not vertex colours.">
                      <span className="ribbon-tool-icon">⌐</span><span className="ribbon-tool-label">Pos Map</span>
                    </button>
